...
2. Head into ``ncsmc_python``, where you'll see a bunch of .py files. You only
   need to worry about one of them for now, ``process_ncsmc_output.py``.

   Open that one up and take a look!

3. You should see some parameters to edit::
   ...
   I assume those parameters are mostly self-explanatory, but we all
   know what happens when I assume.

   They're mostly to do with paths to ncsmc output. The ``Nmax_list`` up top
...
4. Once you've edited those parameters, run the file::
...
5. By this point in the code, here's what's been done:
   ...
   where X is replaced by the first Nmax value of course.
...
6. Soon after you hit enter, you should see something like this:
...
7. When you're done the last Nmax value, wait for a few seconds,
...
8. Give youself a medal, you did it!
